		...
		if (label.equalsIgnoreCase("tokens")) {
			...
			if (args.length == 3) {
				if (args[0].equalsIgnoreCase("set")) {
					...
					setTokens("", amount);
					//target.sendMessage(getPrefix() + ChatColor.GOLD + "Your tokens have been set 
					...
					int amount = pharseInt(args[2]);
					addTokens("", amount);
					//target.sendMessage(getPrefix() + ChatColor.GOLD + "You have been given " + 
					//ChatColor.DARK_RED + getTokens(playerName) + ChatColor.GOLD + " Tokens! Total: " 
					...
					if(hasEnough("", amount)){
						removeTokens("", amount);
						//target.sendMessage(getPrefix() + ChatColor.DARK_RED + getTokens(playerName) 
						//+ ChatColor.GOLD + " have been taken from your account! Total: " + 
						//ChatColor.DARK_RED + amount + ChatColor.GOLD + "!");
					}else{
						//CONSOLE.sendMessage(getPrefix() + ChatColor.GOLD + "That player does not have enough 
